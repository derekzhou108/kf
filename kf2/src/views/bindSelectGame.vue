<template>
  <div class="selectGame u-scrolling">
    ...
    <div class="selectGame-list">
      <div class="selectGame-item" :class="{ active: selectGame === key }" v-for="(item, key) in DONE_APP_LIST" :key="key" @click="selectGameFunc(item, key)">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        selectGame: -1, // -1没有选
        system: -1, //  -1 没有选，1 IOS， 2 安卓
        platform: -1, //-1 没有选，1 凯撒官方， 2 应用宝， 3 其他
    ...
    methods: {
      next() {
        let appId
        if (this.system === 1) {
          appId = this.selectGameItem.app_list.ios
        } else {
          appId = this.selectGameItem.app_list.android
        }
      ...
      selectGameFunc(item, key) {
        this.selectGame = key
        this.selectGameItem = item
      }
    }
  }
</script>
